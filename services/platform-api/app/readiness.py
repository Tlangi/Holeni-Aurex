...
def _database_checks(settings: Settings, tenant_id: str) -> dict[str, ReadinessCheck]:
    ...
    return {
        ...
        "validated_models": ReadinessCheck(int(models.get("validated_models") or 0) >= 1, "PASS" if int(models.get("validated_models") or 0) >= 1 else "BLOCKED", f"Markets with a validated model: {int(models.get('validated_models') or 0)}/{expected_markets}; one independently qualified market is sufficient for a market-scoped Demo canary"),
        "active_risk_profile": ReadinessCheck(active_risk == 1, "PASS" if active_risk == 1 else "BLOCKED", f"Active risk profiles: {active_risk}"),
        "position_sizing_rules": ReadinessCheck(rule_count >= 1, "PASS" if rule_count >= 1 else "BLOCKED", f"Authoritative broker sizing rules: {rule_count}/{expected_markets}; one market is sufficient only for that market's scoped canary, and fallback increments never qualify"),
