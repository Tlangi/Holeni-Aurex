...
def refresh_daily_risk_ledger(
    ...
    cursor.execute(
        """SELECT TOP (1) equity FROM app.account_snapshots
    ...
    drawdown = (
        max(Decimal("0"), (opening_equity - current_equity) / opening_equity * Decimal("100"))
        if opening_equity > 0 else Decimal("0")
    )
    cursor.execute(
    ...
    daily_return = ((current_equity - opening_equity) / opening_equity * Decimal("100")) if opening_equity else Decimal("0")
    peak_return = ((peak_equity - opening_equity) / opening_equity * Decimal("100")) if opening_equity else Decimal("0")
    intraday_drawdown = ((peak_equity - current_equity) / peak_equity * Decimal("100")) if peak_equity else Decimal("0")
    ...
    protection_state = "DAILY_TARGET_LOCKED" if daily_return >= lock_at else "PROFIT_PROTECTION" if daily_return >= protect_at else "NORMAL"
    reason = None
    if not policy:
    ...
    cursor.execute(
        """MERGE app.daily_risk_ledger AS target
           ...
           WHEN MATCHED THEN UPDATE SET current_equity_zar=%s,unrealized_pnl_zar=%s,
             ...
             profit_protection_state=%s,status=%s,status_reason=%s,
           ...
           WHEN NOT MATCHED THEN INSERT
             (daily_risk_ledger_id,tenant_id,trading_account_id,ledger_date_sast,
              ...
              peak_daily_return_pct,profit_protection_state,status,status_reason,last_reconciled_at_utc)
           VALUES(%s,%s,source.trading_account_id,source.ledger_date_sast,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s);""",
        (
