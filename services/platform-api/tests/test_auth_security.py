...
def test_auth_attempt_hash_is_normalized_peppered_and_non_reversible() -> None:
    first = Settings(sql_database="ForexSaas", auth_hash_pepper="pepper-one")
    second = Settings(sql_database="ForexSaas", auth_hash_pepper="pepper-two")
...
def test_rate_limits_and_timezone_safe_account_lockout() -> None:
    settings = Settings(auth_max_email_failures=5, auth_max_address_failures=20)
